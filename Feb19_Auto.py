...
##
## In Feb5.py, plotting of phase center variation is implemented.
################################################################################
import math
import os
...
class Ui_PhaseCenter(QMainWindow):
    def setupUi(self, PhaseCenter):
        ...
        self.ffData = 0
        self.hfss = Hfss(specified_version="2023.1", new_desktop_session=False, non_graphical=False)#,projectname="D:\\phase-center\\Patch_Radome_Scan60.aedt")
        self.cuts = {"Phi": {}, "Theta": {}}
        ...
        PhaseCenter.resize(650, 700)#(731, 674)
        ...
        # self.label_Origin.setGeometry(QRect(10, 55+35+comboBoxHeight+10, 49, 16))
        #
        # self.label_OriginX = QLabel(self.groupBox_UserInput)
        # self.label_OriginX.setObjectName(u"label_OriginX")
        # self.label_OriginX.setGeometry(QRect(60-5, 55+35+comboBoxHeight+10, 16, 16))
        #
        # self.lineEdit_OriginX = QLineEdit(self.groupBox_UserInput)
        # self.lineEdit_OriginX.setObjectName(u"lineEdit_OriginX")
        # self.lineEdit_OriginX.setGeometry(QRect(60+18-10, 55 + 2 * comboBoxHeight + 2 * 10, origin_width, 21))
        # self.lineEdit_OriginX.setValidator(regValidator)
        # self.lineEdit_OriginX.setEnabled(True)
        #
        # self.label_OriginY = QLabel(self.groupBox_UserInput)
        # self.label_OriginY.setObjectName(u"label_OriginY")
        # self.label_OriginY.setGeometry(QRect(60+18+origin_width+3-10, 55+35+comboBoxHeight+10, 16, 16))
        #
        # self.lineEdit_OriginY = QLineEdit(self.groupBox_UserInput)
        # self.lineEdit_OriginY.setObjectName(u"lineEdit_OriginY")
        # self.lineEdit_OriginY.setGeometry(QRect(60+18+1.5*origin_width-10, 55 + 2 * comboBoxHeight + 2 * 10, origin_width, 21))
        # self.lineEdit_OriginY.setValidator(regValidator)
        # self.lineEdit_OriginY.setEnabled(True)
        #
        #
        # self.label_OriginZ = QLabel(self.groupBox_UserInput)
        # self.label_OriginZ.setObjectName(u"label_OriginZ")
        # self.label_OriginZ.setGeometry(QRect(60+18+2*origin_width+3+16-10, 55+35+comboBoxHeight+10, 16, 16))
        #
        # self.lineEdit_OriginZ = QLineEdit(self.groupBox_UserInput)
        # self.lineEdit_OriginZ.setObjectName(u"lineEdit_OriginZ")
        # self.lineEdit_OriginZ.setGeometry(QRect(60+18+3*origin_width-10, 55 + 2 * comboBoxHeight + 2 * 10, 31, 21))
        ...
        self.comboBox_ViewOrientation.setGeometry(QRect(15, 80, 60+30, comboBoxHeight))
        ...
        self.comboBox_referenceCS1.setGeometry(QRect(15, 50, 60+30, comboBoxHeight))
        ...
        self.pushButton_View.setGeometry(QRect(30, 110, 60, 24))
        ...
        # self.pushButton_Clear_Plot.setGeometry(QRect(260, 330, 70, 24))
        # self.pushButton_Clear_Plot.clicked.connect(self.clearPlots)
        # self.pushButton_Clear_Plot.setEnabled(False)
        ...
        self.label_Modeler0.setGeometry(QRect(360, 50, 300*(2/3), 167))
        ...
        self.label_Modeler1.setGeometry(QRect(360, 50+167+25, 300 * (2 / 3), 167))
        ...
        self.label_Modeler2.setGeometry(QRect(360, 50+167*2+25*2, 300 * (2 / 3), 167))
        ...
        self.label_PhC.setGeometry(QRect(120, 270, 130, 16))

        self.comboBox_PhC = QComboBox(self.centralwidget)
        self.comboBox_PhC.setObjectName(u"comboBox_PhC")
        self.comboBox_PhC.setGeometry(QRect(50, 300, comboBoxWidth*5, comboBoxHeight))
        self.comboBox_PhC.setToolTip("Phase Center Value for a given frequency")

        self.label_PlotTitle = QLabel(self.centralwidget)
        self.label_PlotTitle.setObjectName(u"label_PlotTitle")
        self.label_PlotTitle.setGeometry(QRect(120, 330, 170, 16))
    ...
    def retranslateUi(self, PhaseCenter):
        ...
        self.comboBox_ViewOrientation.setItemText(0, QCoreApplication.translate("PhaseCenter", u"Isometric", None))
        self.comboBox_ViewOrientation.setItemText(1, QCoreApplication.translate("PhaseCenter", u"+Y", None))
        self.comboBox_ViewOrientation.setItemText(2, QCoreApplication.translate("PhaseCenter", u"-Y", None))
        ...
        self.comboBox_pcv.setItemText(0, QCoreApplication.translate("PhaseCenter", u"No", None))
        self.comboBox_pcv.setItemText(1, QCoreApplication.translate("PhaseCenter", u"Yes", None))

        #if QT_CONFIG(tooltip)
        self.comboBox_unit.setToolTip("")
        self.comboBox_pcv.setToolTip("If yes, calculates phase center variation wrt frequency, else wrt solution frequency.")
        ...
        self.label_pcv.setText(QCoreApplication.translate("PhaseCenter", u"PCV", None))
        ...
        self.label_PhC.setText(QCoreApplication.translate("PhaseCenter", u"Phase Center Location", None))
        ...
        self.label_forModeler0.setText(QCoreApplication.translate("PhaseCenter", u"Global Isometric View", None))
        self.label_forModeler1.setText(QCoreApplication.translate("PhaseCenter", u"Global Front View", None))
        self.label_forModeler2.setText(QCoreApplication.translate("PhaseCenter", u"Global Top View", None))
        self.label_PlotTitle.setText(QCoreApplication.translate("PhaseCenter", u"", None))
    ...
    def autoCreateRCS(self, setup):
        ...
        cs0 = self.hfss.modeler.create_coordinate_system(name=pccs_auto_name, origin=origin, reference_cs='Global')
        self.hfss.insert_infinite_sphere(definition='Theta-Phi', x_start=-180,
                                         x_stop=180, x_step=self.theta_step,
                                         y_start=0, y_stop=180, y_step=self.phi_step, units='deg',
                                         custom_radiation_faces=None, custom_coordinate_system=pccs_auto_name,
        ...
        theta_at_max_rETotal =   float("".join(b).split()[0]) # ['40','-80']
        phi_at_max_rETotal  =   float("".join(b).split()[1])
        # making it separate check for trying different approach
        ...
        for i,x in enumerate(b):
            if not x.isdigit() and x!='-' and x!='.':
                b[i]=' '
        # below two are float values of theta and phi at which the max rE field occurs
        theta_at_max_rEcomp = float("".join(b).split()[0]) # ['40','-80']
        ...
        self.hfss.insert_infinite_sphere(definition='Theta-Phi', x_start=-self.thetascale*self.theta_step, x_stop=self.thetascale*self.theta_step, x_step=self.thetascale*self.theta_step,
        ...
        variations["Theta"] = [str(-self.thetascale * self.theta_step) + 'deg', '0deg', str(self.thetascale * self.theta_step) + 'deg']
        ...
        ffData1 = self.hfss.post.get_solution_data(expressions="rE" + polarization,
                                              setup_sweep_name=setup, primary_sweep_variable = "Theta", report_category="Far Fields", \
                                              context=context, variations=variations)
        del variations["Phi"]
        variations["Phi"] = ['90deg']
        ffData2 = self.hfss.post.get_solution_data(expressions="rE" + polarization,
                                              setup_sweep_name=setup, primary_sweep_variable = "Theta", report_category="Far Fields", \
                                              context=context, variations=variations)

        freq_unit = ffData1.units_sweeps["Freq"]
        factor = self.getFreqMultiplier(freq_unit)
        k0 = 2 * math.pi * ffData1.intrinsics["Freq"][0] * factor / 299792458
        x0, y0, z0 = self.calculatePC2(k0, ffData1, ffData2)

        x0 = self.convertMeterToUnit(x0, unit)
        y0 = self.convertMeterToUnit(y0, unit)
        z0 = self.convertMeterToUnit(z0, unit)
        self.hfss.modeler.create_coordinate_system(name="PhC_"+current_freq, origin=[str(x0) + unit, str(y0) + unit, str(z0) + unit], \
        ...
        self.hfss.insert_infinite_sphere(definition='Theta-Phi', x_start=-1*self.plotRange, x_stop=self.plotRange,
                                        ...
                                        y_start=0, y_stop=0, y_step=0, units='deg',
        ...
        self.plotwidget.plot(cut_data_phi0.primary_sweep_values, cut_data_phi0.data_phase(radians=False),
                             ...
                             name=cutplane + "=" + "0" + "deg" + " | " + "PhC_"+current_freq + " | \u03C3=" + str("{:.2f}".format(np.std(cut_data_phi0.data_phase(radians=False)))))
        self.numCurves += 1
        return PhC_wrt_global
    ...
    def createRCS(self):
        ...
        if self.Auto:
            ...
            if self.comboBox_pcv.currentText() == "Yes":
                ...
                self.label_PlotTitle.setText("Phase Center Variation vs Freq")
                ...
                f_unit = f00[-3:]
                m = self.getFreqMultiplier(f_unit)
                f0 = self.getFrequency(f00) / m
                f1 = self.getFrequency(f11) / m
                ...
                if self.hfss.setups[0].sweeps[0].props['RangeType']=='LinearCount':
                    ...
                    n = math.floor((f1 - f0)*m/self.getFrequency(str_rangestep))+1
                ...
                #current_freq[:-3]
                aa = self.autoCreateRCS(setup)
                answer = "Freq="+self.lineEdit_Frequency.text()+" | x="+str("{:.2f}".format(aa[0][0]))+" | y="+str("{:.2f}".format(aa[1][0]))+" | z="+str("{:.2f}".format(aa[2][0]))
                self.comboBox_PhC.addItem(answer)
            return
    ...
    def calculatePC2(self, k0, ffData1, ffData2):
        ...
        :param cut_data: contains
        :param A:
        :param p:
        :return:
        '''
        theta1 = ffData1.intrinsics["Theta"][0]*math.pi/180
        theta2 = ffData1.intrinsics["Theta"][1]*math.pi/180
        theta3 = ffData1.intrinsics["Theta"][2]*math.pi/180

        # phase1_atphi_90 = ffData2.data_phase()[0] if ffData2.data_phase()[0]>=0 else math.pi+ffData2.data_phase()[0]
        # phase2_atphi_90 = ffData2.data_phase()[1] if ffData2.data_phase()[1]>=0 else math.pi+ffData2.data_phase()[1]
        # phase3_atphi_90 = ffData2.data_phase()[2] if ffData2.data_phase()[2]>=0 else math.pi+ffData2.data_phase()[2]

        expression = ffData2.active_expression
        phase1_atphi_90 = [math.atan2(k, i) for i, k in zip(ffData2.data_real(expression), ffData2.data_imag(expression))][0]
        phase2_atphi_90 = [math.atan2(k, i) for i, k in zip(ffData2.data_real(expression), ffData2.data_imag(expression))][1]
        phase3_atphi_90 = [math.atan2(k, i) for i, k in zip(ffData2.data_real(expression), ffData2.data_imag(expression))][2]
        # diagnostics
        [print("phase negative!") if ffData2.data_phase()[i] < 0 else print("positive") for i in [0, 1, 2]]
        [print("phase negative!") if ffData1.data_phase()[i] < 0 else print("positive") for i in [0, 1, 2]]
        ...
        phase12 = phase1_atphi_90 - phase2_atphi_90
        phase13 = phase1_atphi_90 - phase3_atphi_90
        ...
        # phase1 = ffData1.data_phase()[0] if ffData1.data_phase()[0]>=0 else math.pi+ffData1.data_phase()[0]
        # phase2 = ffData1.data_phase()[1] if ffData1.data_phase()[1]>=0 else math.pi+ffData1.data_phase()[1]
        # phase3 = ffData1.data_phase()[2] if ffData1.data_phase()[2]>=0 else math.pi+ffData1.data_phase()[2]
        expression = ffData1.active_expression
        phase1 = [math.atan2(k, i) for i, k in zip(ffData1.data_real(expression), ffData1.data_imag(expression))][0]
        phase2 = [math.atan2(k, i) for i, k in zip(ffData1.data_real(expression), ffData1.data_imag(expression))][1]
        phase3 = [math.atan2(k, i) for i, k in zip(ffData1.data_real(expression), ffData1.data_imag(expression))][2]
